        ...
        private class Context
        {
            ...
            public Context(string name, BlockList blocks, CIL.ModuleBuilder moduleBuilder)
            {
                ...
                _method = _typeBuilder.DefineMethod(name, MethodAttributes.Public | MethodAttributes.Static,
                    typeof(void), new Type[] { typeof(Register.Context).MakeByRefType(), typeof(SystemCallbacks),
            ...
            // Stack state:
            // X < Top
            private void EmitLoadSystemCallbacks() =>
                _generator.Emit(CIL.OpCodes.Ldarg_1); // Load callbacks argument
            ...
            {
                _generator.Emit(CIL.OpCodes.Ldc_I4, (int)constant); // Load constant
                EmitStoreRegister(id); // Store into register
            }
            ...
            void InstructionVisitor(Instruction.Instruction instr)
            {
                switch (instr.Primary)
                {
                    case OpCode.Primary.Secondary0:
                        {
                            ...
                            switch (secondary)
                            {
                                case OpCode.Secondary0.Clr: // clear()
                                    EmitLoadSystemCallbacks();
                                    _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("Clear"));
                                    break;
                                default:
                                    break;
                            }
                    ...
                    case OpCode.Primary.Secondary8:
                        {
                            ...
                            switch (secondary)
                            {
                                ...
                                case OpCode.Secondary8.Add: // VF = (VX + VY) > 0xff; VX = VX + VY
                                    ...
                                    // Set flags
                                    _generator.Emit(CIL.OpCodes.Ldc_I4, (int)0xff); // Load 0xff onto stack
                    ...
                    case OpCode.Primary.Rand: // VX = rand() & KK
                        ...
                        _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("Random"));
                    ...
                    case OpCode.Primary.Draw: // VF = draw(VX, VY, I, N)
                        ...
                        // Returns VF state
                        _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("Draw"));
                    ...
                    case OpCode.Primary.SecondaryF:
                        {
                            ...
                            switch (secondary)
                            {
                                case OpCode.SecondaryF.MoveD: // VX = DT
                                    EmitLoadSystemCallbacks();
                                    _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("GetDelayTimer"));
                                    EmitStoreRegister(instr.Param.X);
                                    break;
                                case OpCode.SecondaryF.KeyD: // VX = waitKey()
                                    EmitLoadSystemCallbacks();
                                    _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("WaitKey"));
                                    EmitStoreRegister(instr.Param.X);
                                    break;
                                case OpCode.SecondaryF.LoadD: // DT = VX
                                    EmitLoadSystemCallbacks();
                                    EmitLoadRegister(instr.Param.X);
                                    _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("SetDelayTimer"));
                                    break;
                        ...
                        case OpCode.Primary.SecondaryE:
                            {
                                ...
                                switch (secondary)
                                {
                                    case OpCode.SecondaryE.Skpr: // SKIP if PRESSED[X]
                                        EmitLoadSystemCallbacks();
                                        EmitLoadRegister(instr.Param.X);
                                        _generator.Emit(CIL.OpCodes.Call, typeof(SystemCallbacks).GetMethod("IsKeyPressed"));
