...
namespace Chip8_CIL
{
    public class Translator
    {
        ...
        public delegate void ProgramDelegate(ref Register.Context ctx, SystemCallbacks systemCallbacks, byte[] memory, ushort[] returnAddressStack);

        public Translator(byte[] memory)
        {
            _memory = memory;
        ...
        {
            Logger.StartFunction("DiscoverBlocks", Logger.Level.Debug);
            ...
            while (currentBlock != null)
            {
                ...
                Logger.LogTrace("Addr: {0:x} Raw: {1:x}, Primary: {2:}, NNN: {3:x}, N: {4:x}, X: {5:x}, Y: {6:x}, KK: {7:x}", instrAddr, instr.Raw, instr.Primary.ToString(), instr.Param.NNN, instr.Param.N, instr.Param.X, instr.Param.Y, instr.Param.KK);
                ...
                {
                    Logger.LogVerbose("Leaf at: {0:x}", instrAddr);
        ...
        public ProgramDelegate TranslateProgram(ushort startAddr)
        {
            ...
            if (Settings.DumpEnabled)
                File.WriteAllText(Settings.GetDumpPathFor(string.Format("PROG_{0:X}.dot", startAddr)), blocks.GetGraphDotString());
